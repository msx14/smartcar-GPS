...
int FirstRowProcess(void)
{
	...
	if (LEFT_EAGE + LOST_FTH > LL[DOWN_EAGE] && RIGHT_EAGE - LOST_FTH < RL[DOWN_EAGE])	//两侧丢边 判定为双侧十字
	{
	...
	else if (LEFT_EAGE + LOST_FTH > LL[DOWN_EAGE])	//左侧丢边
	{
		...
		for (i = DOWN_EAGE - 1; i > DOWN_EAGE - UP_TH; i--)
		{
			...
			if (RL[i] > RIGHT_EAGE - LOST_FTH)		//Lost line
			{
	...
	else if (RIGHT_EAGE - LOST_FTH < RL[DOWN_EAGE])		//右侧丢边
	{
		unsigned char LeftLostFlag = 0, RightLostFlag = 0;  //the flag of lost eage
		//Lost Line Process
		int i;
		...
		for (i = DOWN_EAGE - 1; i > DOWN_EAGE - UP_TH; i--)
		{
			...
			if (LL[i] < LEFT_EAGE + LOST_FTH)		//Lost line
			{
				if (NoLostFlag)			//other side is no lost
				{
					LeftPnt.ErrRow = DOWN_EAGE;
