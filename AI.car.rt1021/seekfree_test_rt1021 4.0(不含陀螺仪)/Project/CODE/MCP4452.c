...
int resistence_protect(int resval)
{
        if (resval > RES_MAX)     resval = RES_MAX;
      else if (resval < RES_MIN)     resval = RES_MIN;
	return resval;

}
void MCP4452_ADC_self_steady()
{
	if(gpio_get(D14)==0&&adc_read==0)  
	{
     if((Exp_LO_L-LO_L)>0)   W_LO--; else  W_LO++;
	   if((Exp_LI_L-LI_L)>0)   W_LI--; else  W_LI++;
	   if((Exp_M_L-M_L)>0)   W_M--; else  W_M++;
	   if((Exp_RI_L-RI_L)>0)   W_RI--; else  W_RI++;
	   if((Exp_RO_L-RO_L)>0)   W_RO--; else  W_RO++;
	   if((Exp_belly_R_L-belly_R_L)>0)   W_belly_R--; else  W_belly_R++;
	   if((Exp_belly_L_L-belly_L_L)>0)   W_belly_L--; else  W_belly_L++;
	
	
	//*******限幅*********
  W_LO= resistence_protect(W_LO);
  W_LI= resistence_protect(W_LI);
	W_M= resistence_protect(W_M);
	W_RI= resistence_protect(W_RI);
	W_RO= resistence_protect(W_RO);
	W_belly_R= resistence_protect(W_belly_R);
	W_belly_L= resistence_protect(W_belly_L);
	MCP4452_val_ALL_init( initR, W_RO, W_belly_R, W_RI,W_M, W_LO, W_LI, W_belly_L);
	}
}