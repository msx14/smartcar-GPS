...
void Key_Function(void)
{
	...
	else
	{
		switch (g_Key)
		{
		case 1: /*MyFlash_Write(0);*/ stop_tiktok=0;global_judge=0;   break;    //写入FLASH
...
void Main_Show(void)
{
	g_pageCount = 0;			
		...
		Insert_Page("out_ADC");
			Insert_Float("outL_L",&outL_L);
			Insert_Float("outM_L",&outM_L);
			Insert_Float("outR_L",&outR_L);
			Insert_Float("outL_r1",&outL_L_r1);
			Insert_Float("outM_r1",&outM_L_r1);
			Insert_Float("outR_r1",&outR_L_r1);
		...
		Insert_Page("in_ADC");
	  	...
	  	Insert_Page("QuickChange");
				...
				Insert_Int("angle",&smotor_angle);//舵机打角
				Insert_Int("adc",&adc_read);//舵机打角
	      Insert_Float("g_steer_error",&g_steer_error);
				Insert_Int("stop_judge",&stop_judge);//控制模式
				
			Insert_Page("long");
				Insert_Float("l_r_p",&g_steer_p_right);//舵机右pid
				Insert_Float("l_r_d",&g_steer_d_right);//舵机右pid
				Insert_Float("l_l_p",&g_steer_p_left);//舵机右pid
				Insert_Float("l_l_d",&g_steer_d_left);//舵机右pid
				Insert_Int("k",&KDet);							//差速系数k	
								
			Insert_Page("short");
				Insert_Float("s_r_p",&s_steer_p_right);//舵机右pid
				Insert_Float("s_r_d",&s_steer_d_right);//舵机右pid
				Insert_Float("s_l_p",&s_steer_p_left);//舵机右pid
				Insert_Float("s_l_d",&s_steer_d_left);//舵机右pid
