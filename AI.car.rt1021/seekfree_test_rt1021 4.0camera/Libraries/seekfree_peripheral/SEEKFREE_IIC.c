...
static uint16 simiic_delay_time=100;   //使用小钻风摄像头时，此延时需要设置为2000左右  //ICM等传感器应设置为100
...
uint16 simiic_read_reg2(uint8 dev_add, uint8 reg, IIC_type type)
{
	uint16 dat;
	simiic_start();
    send_ch( (dev_add<<1) | 0x00);  //发送器件地址加写位
