...
//	uint8 sendbuff[32] = { 0 };
//	uint8 temp = 0;
//	int16 yaww, rolll, pitchh; //??D?????
//	int32 alt;
//	alt = 0;//z_est[0]*100;
//	yaww = (int16)(imu_data.yaw * 100);
//	rolll = (int16)(imu_data.rol * 100);
//	pitchh = (int16)(imu_data.pit * 100);
...
//	}
//	uart_putbuff(USART_3, sendbuff, 17);
}
...
void UserData(void)
{
	...
	uart_putbuff(USART_3, testdatatosend, _cnt);//调用发送数据函数
}
...
*/
uint8 com_rx_buffer;
lpuart_transfer_t   com_receivexfer;
lpuart_handle_t     g_com_lpuartHandle;
int get_times = 0;

//void com_call_back(LPUART_Type *base, lpuart_handle_t *handle, status_t status, void *userData)
//{
//	if(kStatus_LPUART_RxIdle == status)
//	{
//		camera_error = com_rx_buffer;
//		get_times ++;
//	}
//	handle->rxDataSize = com_receivexfer.dataSize;  //还原缓冲区长度
//  handle->rxData = com_receivexfer.data;          //还原缓冲区地址
//}


//void com_init(void)
//{
//	uart_init (COM_COF_UART, 115200,COM_COF_UART_TX,COM_COF_UART_RX);
//	uart_rx_irq(COM_COF_UART, 1);
//	com_receivexfer.dataSize = 1;
//	com_receivexfer.data = &com_rx_buffer;
//	
//	uart_set_handle(COM_COF_UART, &g_com_lpuartHandle, com_call_back, NULL, 0, com_receivexfer.data, 1);
//}
