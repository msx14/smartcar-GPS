...
//	int16 temp;
	...
	CI_GetModelInfoXIP(model1, &inf);
			...
			smotor_angle = (int32)g_servoValue * 400 / 128;//放大数据
...
//			 	 PRINTF("temp:%d.\r\r",temp);
	  	 pwm_duty(PWM1_MODULE0_CHB_C27,SMOTOR_CENTER+smotor_angle);
			...
			if(g_convert_flag == 3)		
				pwm_duty(PWM1_MODULE0_CHB_C27,SMOTOR_CENTER+smotor_angle);  
//			 PRINTF("g_servoValue:%d.\r\r",temp);
//	 	 PRINTF("smotor_angle:%d.\r\r",smotor_angle);
//			 	 PRINTF("g_out[0]:%d.\r\r",g_out[0]);
}
...
// 			CIETest(model1,&tstCtx);	
    while (1)
    {
			 if(uart_query(USART_1,&dat))
			 {
			    PRINTF("SUCCESS");
			 }				 
			stop_judge = gpio_get(B14);
			MCP4452_ADC_self_steady();	
//      PRINTF("camera error:%d",camera_error);
			if(gpio_get(D15)==0)		Menu();
			else		{
//				           
//				PRINTF("DATA:%d",dat);
//				uart_putchar(USART_3,(unsigned char)(camera_error));
//				PRINTF("camera_error:%d",camera_error);
				//UserData();
				//UserData();//
			//	PRINTF("camera error:%d, get_time:%d\n",camera_error, get_times);
			//	data_send();
	//		type_change();
			}
		}
