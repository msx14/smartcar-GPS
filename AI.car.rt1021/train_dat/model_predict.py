...
# 将用于测试的训练样本的output与标准值对比，差值逐项累和
...
if __name__ == "__main__":
    ...
    # 语法是 A = reshape（A，m，n）； 或者 A = reshape（A，[m,n]）; 都是将A的行列排列成m行n列

    x_test = x_test.astype('int16')
    print(max(x_test.flatten()))    # flatten使变平成为一个数
    ...
    model = load_model(mode_name)    # 载入model
    ...
    losses, accuracy = model.evaluate(x_input, y_input)    # 输入数据和标签,输出损失和精确度
